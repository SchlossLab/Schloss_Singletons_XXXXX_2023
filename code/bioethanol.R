...
make_files_file <- function(){
	...
	mimarks <- read.table(file="data/bioethanol/SRP055545_info.tsv", header=T,
	 											stringsAsFactors=FALSE, sep='\t')

	sample_map <- mimarks$Sample_Name
	names(sample_map) <- mimarks$Run

	read_1 <- list.files(path="data/bioethanol/", pattern="*1.fastq.gz")
	...
	write.table(files, "data/bioethanol/bioethanol.files", row.names=F, col.names=F, quote=F, sep='\t')
