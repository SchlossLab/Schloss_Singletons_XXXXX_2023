...
a_sequences <- rep(full_dataset$sequences, full_dataset$n_seqs) %>% sample(., sum(a$N))
a_groups <- rep(a$group, a$N)

b_sequences <- rep(sub_dataset$sequences, sub_dataset$n_seqs) %>% sample(., sum(b$N))
...
# Output design file
group_order_count %>% select(group, grouping) %>% write_tsv(paste0(path, "/data.", seed, ".", min_class, ".design"), col_names=F)
