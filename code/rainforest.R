...
make_files_file <- function(){
	...
	mimarks <- read.table(file="data/rainforest/ERP023747_info.tsv", header=T,
	 											stringsAsFactors=FALSE, sep='\t')

	sample_map <- mimarks$Alias
	names(sample_map) <- mimarks$Run

	read_1 <- list.files(path="data/rainforest/", pattern="*1.fastq.gz")
	...
	write.table(files, "data/rainforest/rainforest.files", row.names=F, col.names=F, quote=F, sep='\t')
