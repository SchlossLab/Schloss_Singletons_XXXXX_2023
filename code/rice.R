...
make_files_file <- function(){
	...
	mimarks <- read.table(file="data/rice/SRP044745_info.csv", header=T,
	 											stringsAsFactors=FALSE, sep='\t')

	sample_map <- mimarks$Sample_Name
	names(sample_map) <- mimarks$Run

	read_1 <- list.files(path="data/rice/", pattern="*1.fastq.gz")
	...
	write.table(files, "data/rice/rice.files", row.names=F, col.names=F, quote=F, sep='\t')
