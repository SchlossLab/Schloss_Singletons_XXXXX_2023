...
make_files_file <- function(){
	...
	mimarks <- read.table(file="data/seagrass/SRP092441_info.csv", header=T,
	 											stringsAsFactors=FALSE, sep='\t')

	sample_map <- mimarks$Sample_Name
	names(sample_map) <- mimarks$Run

	read_1 <- list.files(path="data/seagrass/", pattern="*1.fastq.gz")
	...
	write.table(files, "data/seagrass/seagrass.files", row.names=F, col.names=F, quote=F, sep='\t')
