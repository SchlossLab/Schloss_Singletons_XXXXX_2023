...
make_files_file <- function(){
	...
	mimarks <- read.table(file="data/stream/SRP075852_info.csv", header=T,
	 											stringsAsFactors=FALSE, sep='\t')

	sample_map <- mimarks$Sample_Name
	names(sample_map) <- mimarks$Run

	read_1 <- list.files(path="data/stream/", pattern="*1.fastq.gz")
	...
	write.table(files, "data/stream/stream.files", row.names=F, col.names=F, quote=F, sep='\t')
